...
namespace PSI.Models.VEModels
{
    public class VE_PurchaseWeightNoteValidator : AbstractValidator<VE_PurchaseWeightNote>
    {
        public VE_PurchaseWeightNoteValidator()
        {
            ...
            RuleFor(x => x.CustomerName).NotNull().When(x => x.CustomerId == 0).WithMessage("進貨對象為新客戶時，客戶名稱為必填");
            RuleFor(x => x.CarNoId).NotNull().WithMessage("為必填欄位");
            RuleFor(x => x.HasTax).NotNull().WithMessage("為必填欄位");
            RuleFor(x => x.CarNo).NotNull().When(x => x.CustomerId == 0).WithMessage("為新車牌的時候，車牌名稱為必填");
            RuleFor(x => x.FullWeight.ToString()).NotNull().WithMessage("為必填欄位").Matches(@"^\+?[1-9][0-9]*$").WithMessage("格式不正確，需為大於0的正整數"); // 非0正整數
